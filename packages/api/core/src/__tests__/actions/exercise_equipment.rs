...
pub async fn read_exercise_equipment(server: &MockServer, equipment: &impl Model) -> MockResponse {
    server.get_with_route_key("/api/exercise-equipment", equipment.rk()).await
}
